...
type Usecase interface {
	...
	GetExperienceMasterData(ctx context.Context) ([]*models.MasterDataExperience, error)
	FilterSearchExp(ctx context.Context, isMerchant bool, search, token, status, cityID string, harborsId string, expTypeId string, startDate string, endDate string, guest string, trip string, bottomPrice string, upPrice string, sortBy string, page, limit, offset int,provinceId string,currency string,bookingType string,paymentType string) (*models.FilterSearchWithPagination, error)
