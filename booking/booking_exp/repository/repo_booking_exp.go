...
func (b bookingExpRepository) QueryCountHistoryByUserId(ctx context.Context, userId string, yearMonth string) (int, error) {
	...
	if yearMonth != "" {
		...
		rows, err := b.Conn.QueryContext(ctx, query1, userId, date, date, time.Now().Format("2006-01-02"))
		if err != nil {
			logrus.Error(err)
			return 0, err
		}
		...
		rows2, err := b.Conn.QueryContext(ctx, query2, userId, date, date)
		if err != nil {
			logrus.Error(err)
			return 0, err
		}
		...
		rows, err := b.Conn.QueryContext(ctx, query1, userId,time.Now().Format("2006-01-02"))
		if err != nil {
			logrus.Error(err)
			return 0, err
		}
		...
		rows2, err := b.Conn.QueryContext(ctx, query2, userId)
		if err != nil {
			logrus.Error(err)
			return 0, err
		}
...
func (b bookingExpRepository) QuerySelectIdHistoryByUserId(ctx context.Context, userId string, yearMonth string, limit, offset int) ([]*string, error) {
	...
	if yearMonth != "" {
		...
		if limit != 0 {
			query = query + ` LIMIT ? OFFSET ?`
			rows, err := b.Conn.QueryContext(ctx, query, userId, date, date, time.Now().Format("2006-01-02"),userId, date, date, limit, offset)
			if err != nil {
				logrus.Error(err)
				return nil, err
			}
		...
		if limit != 0 {
			query = query + ` LIMIT ? OFFSET ?`
			rows, err := b.Conn.QueryContext(ctx, query, userId, time.Now().Format("2006-01-02"),userId, limit, offset)
			if err != nil {
				logrus.Error(err)
				return nil, err
			}
