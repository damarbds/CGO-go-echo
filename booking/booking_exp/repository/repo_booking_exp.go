...
func (b bookingExpRepository) GetBookingIdByUserID(ctx context.Context, transactionStatus, bookingStatus int, userId string, limit, offset int) ([]*string, error) {
	...
	if transactionStatus == 1 {
		query = query + ` AND (t.status IN (?, 5) OR t.status IS NULL)`
	} else {
		query = query + ` AND (t.status = ? OR t.status IS NULL)`
	}

	if limit != 0 {
		query = query + ` LIMIT ? OFFSET ?`
		fmt.Println(query)
		rows, err := b.Conn.QueryContext(ctx, query, bookingStatus, userId, transactionStatus, limit, offset)
		if err != nil {
			logrus.Error(err)
			return nil, err
		}
